...
strcat(string, "No of Characters="); sprintf(temp,"%d",i); 
strcat(string,temp); 
strcat(string,","); 
strcat(string, "No of Words="); 
sprintf(temp,"%d",w); 
strcat(string,temp); strcat(string,","); 
strcat(string,"No of Lines="); 
sprintf(temp,"%d",l); 
strcat(string,temp); strcat(string,","); 
c2s = open(C_S,O_RDWR); printf("Writing to myfifo2\n"); write(c2s,string,sizeof(string)); 
fclose(fp); 
close(s2c); 
close(c2s); 
return 0; 
